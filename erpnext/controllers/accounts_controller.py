...
class AccountsController(TransactionBase):
	...
	def validate_multiple_billing(self, ref_dt, item_ref_dn, based_on):
		...
		# periodic-valuation items are billed by QUANTITY, not amount: the receipt
		...
		# validate_kernel_qty_billing (Purchase Invoice only).
		kernel_routed = (
			self.get_kernel_routed_items()
			if self.doctype == "Purchase Invoice" and hasattr(self, "get_kernel_routed_items")
			else set()
		)
		...
		for row in ref_wise_billed_amount.values():
			...
			if overbill_amt > precision_allowance and not is_overbilling_allowed:
				if row.item_code in kernel_routed:
					continue
